...
import type { Scope, ScopeReport, WorkerStatus } from '../lib/types';
...
export function AnalysisPlan({ compact = false }: { compact?: boolean }) {
  ...
  const invalidate = () => { qc.invalidateQueries({ queryKey: ['scope'] }); qc.invalidateQueries({ queryKey: ['jobs-status'] }); qc.invalidateQueries({ queryKey: ['items'] }); qc.invalidateQueries({ queryKey: ['stats'] }); };
  ...
  const jobs = useMutation({ mutationFn: (p: { path: string; body?: any }) => api.post<any>(p.path, p.body), onSuccess: (r, v) => { if (v.path.includes('queue')) toast.success(`Queued ${r.justQueued ?? ''} saves`); if (v.path.includes('reset-quota')) toast.success(`${r.reset} saves moved back to pending`); invalidate(); }, onError: (e: any) => toast.error(e.message) });
  const r = scopeQ.data;
  const w = worker.data;
    ...
    <div className="space-y-4">
      ...
      <div className={cn('card p-4', w?.paused && w.pauseReason && w.pauseReason !== 'manual' && 'border-warn/50')}>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="ml-auto flex flex-wrap items-center gap-2">
            ...
            <button onClick={() => jobs.mutate({ path: '/api/jobs/queue', body: { what: 'eligible' } })} disabled={!r.counts.eligiblePending} className="btn btn-sm btn-primary" title="Queue every eligible save (inside the date window & media types, not excluded)"><Sparkles size={13} /> Analyze eligible ({r.counts.eligiblePending.toLocaleString()}) ≈ {usd(est)}</button>
      ...
      <div className="card p-5">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Field label="Transcribe speech" hint="~30% of the per-reel cost. Short reels carry the most value per cent.">
          ...
          <Field label="Vision frames per reel" hint="Frames the model looks at (on-screen text, visuals). Fewer = cheaper.">
          ...
          <Field label="Quality tier" hint="Standard = gpt-5.4-mini (recommended). Economy = gpt-5.4-nano, ~4× cheaper, slightly blunter summaries.">
