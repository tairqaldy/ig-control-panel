        ...
        <div className="grid md:grid-cols-[minmax(0,0.9fr)_minmax(0,1.4fr)] md:h-[calc(100vh-3rem)] md:max-h-[calc(100vh-3rem)] md:overflow-hidden rounded-2xl">
          ...
          <div className="p-4 md:p-5 md:border-r border-line bg-bg-2/40 md:overflow-y-auto min-h-0">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              ...
              <button onClick={() => patch.mutate({ excluded: !item.excluded }, { onSuccess: () => { toast.success(item.excluded ? 'Included again' : 'Excluded — hidden everywhere, never analyzed'); if (!item.excluded) modal.close(); } })} className={cn('btn', item.excluded && 'chip-active border-accent')} title="Exclude from Resurfly entirely (not analyzed, not searchable, not in the graph). Reversible from Library → Excluded.">{item.excluded ? <Eye size={14} /> : <EyeOff size={14} />}{item.excluded ? 'Include again' : 'Exclude'}</button>
              {item.author && !item.excluded && (
                <button onClick={() => { if (confirm(`Exclude ALL saves by @${item.author} from Resurfly? (reversible)`)) excludeAuthor.mutate(item.author!); }} className="btn col-span-2"><UserX size={14} /> Exclude everything by @{item.author}</button>
              )}
              <button onClick={() => { if (confirm('Delete this save from Resurfly? (Does not touch Instagram)')) del.mutate(); }} className="btn btn-danger col-span-2"><Trash2 size={14} /> Delete</button>
            </div>
          </div>
