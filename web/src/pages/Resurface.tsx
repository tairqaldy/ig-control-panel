  ...
  return (
    <div>
      ...
      {q.isLoading ? (
        ...
        <Empty icon={<Heart size={26} />} title="Nothing to resurfly yet" body="Once a handful of saves are analyzed, this page will pick three every day." action={<button onClick={() => qc.invalidateQueries({ queryKey: ['resurface'] })} className="btn">Refresh</button>} />
      )}
    </div>
  );
}
