...
4. **Set `META_APP_LIVE=true`** in the server's environment. Meta exposes no "is this app live" flag on the API, so this is how the product learns it. Until it is set, Resurfly assumes Development mode and hides the Connect button from everyone without a tester role — onboarding, Automations, Analytics and the AI all read `GET /api/instagram/availability` before they offer a connection, so that nobody is sent at a door that cannot open.
